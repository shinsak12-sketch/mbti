...
| `/pan` (`pan.html`) | 🪙 **판치기**: 시작 누르고 폰을 위로 탁(가속도 센서) → 100원 동전 튀기기 · 한 사람 한 번 · 뒤집힌 개수 기록 · 판 밖은 무효 · 센서 없으면 화면 꾹 누르기 |
| `/race` (`race.html`) | 🏁 **점심 경주**: 16종 캐릭터 랜덤 출전 · 특성 속도곡선(기대값 동일) + 랜덤 사고 이벤트 시뮬레이션 · 슬로모션·사진 판정 · 꼴등/1등 선택 |
| `/archery` (`archery.html`) | 🏹 **양궁**: 아래로 끌어 시위 당기기 · 세게·오래 당길수록 조준 떨림 · 약하면 처짐 · 1~5발 합산 · 근/중/원거리 |
